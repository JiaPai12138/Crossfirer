...
# 第一分析进程
def detection1(que, array):
    Analysis1 = FrameDetection(1, array[0], 2)
    while True:
        if not que.empty():
            img1 = que.get_nowait()
            que.task_done()
            array[1] = 1
            Analysis1.detect(img1)
        array[1] = 0
        sleep(0.001)
...
def detection2(que, array):
    Analysis2 = FrameDetection(1, array[0], 2)
    while True:
        if not que.empty() and array[1]:
            img2 = que.get_nowait()
            que.task_done()
            array[2] = 1
            Analysis2.detect(img2)
        array[2] = 0
        sleep(0.001)
