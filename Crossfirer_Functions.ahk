...
ShowHelp(XGui, YGui, Gui_Number) ;ok
{
    global Need_Help := !Need_Help
    If Need_Help
...
ProcessExist(Process_Name) ;ok
{
    Process, Exist, %Process_Name%
    Return Errorlevel
}

ChangeMode(Gui_Number1, Gui_Number2, ModeID, StatusID, ByRef AutoMode, XGui1, YGui1, XGui2, YGui2, CrID, Xch, Ych) ;ok
{
    AutoMode := !AutoMode
...
AutoFire(mo_shi, Gui_Number1, Gui_Number2, ModeID, StatusID, game_title, XGui1, YGui1, XGui2, YGui2, CrID, Xch, Ych) ;ok
{
    ...
    {
        Var := W1 // 2 - 5 ;798
        UpdateText(Gui_Number2, StatusID, "搜寻敌人", XGui2, YGui2) ;war zone, need less HyperSleep 
        ...
        Loop ;detect color in three lines where shows the enemy name
        {
            ...
            While Shoot_Time(X1, Y1, Var, game_title) ;if detected color is found in string
            {
                ...
                Switch mo_shi
                {
                    ...
                    Case 8:
                        If Not (GetColorStatus(X1, Y1, 955, 483, PosColor_snipe) || GetColorStatus(X1, Y1, 804, 600, PosColor_snipe))
                        {
        ...
        ;HyperSleep(1) ;trying to trick anticheat with HyperSleep
    }
}
...
Shoot_Time(X, Y, Var, game_title) ;ok
{
...
C4Timer(XGuiC, YGuiC, ByRef C4_Start, ByRef C4_Time, Gui_Number, ControlID) ;精度0.1s Loop type
{
    WinGetPos, X1, Y1, , , ahk_class CrossFire
    If Is_C4_Time(X1, Y1)
    {
        ...
        Else If C4_Start > 0
        {
            C4_Time := SubStr("00" . Format("{:.0f}", (40 - (SystemTime() - C4_Start) / 1000)), -1) ;to force two digit integer
...
SetGuiPosition(ByRef XGui, ByRef YGui, GuiPosition, OffsetX, OffsetY) ;ok
{
    ...
    If InStr("H", GuiPosition)
    {
        XGui := X1 + W1 // 2 + OffsetX
        YGui := Y1 + OffsetY
    }
    Else If InStr("V", GuiPosition)
    {
        XGui := X1 + 2 + OffsetX
        YGui := Y1 + H1 // 2 + OffsetY
    }
    Else If InStr("M", GuiPosition)
    {
        XGui := X1 + W1 // 2 + OffsetX
...
UpdateText(Gui_Number, ControlID, NewText, X, Y) ;ok
{ ;Copy From AHK Windows Spy, preventing periodic flickering
    static OldText := {}
