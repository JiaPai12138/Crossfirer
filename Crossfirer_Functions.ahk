...
CheckPosition(ByRef Xcp, ByRef Ycp, ByRef Wcp, ByRef Hcp)
{
    ...
    ;FileAppend, %Xcp% %Ycp% %Wcp% %Hcp% clientgetpos`n, %A_ScriptDir%\log.txt
    ...
    ;FileAppend, %Mon_Width% %Mon_Hight% Resolution`n, %A_ScriptDir%\log.txt

    If (Wcp >= Mon_Width) || (Hcp >= Mon_Hight) ;全屏检测
    {
        ;Wcp := Mon_Width
        ;Hcp := Mon_Hight
